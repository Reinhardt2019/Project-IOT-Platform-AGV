...
#选择markerArray的index,count表示总目标点计数，index表示已完成的目标点计数
#当count==3,markerArray==[0,1,2],此时路径为010212
def index_selector(count,index):
    result = 0
    #if (count < 2):
    #    return
    if (index%3 == 1):
	result = 1
    #if index%3==0 or 2, 当前小车返回出发点或去下一个出发点
    else:
        result = index//3
	#if result==1,let result==2
        if (result >= 1):  
            result += 1
    return result
...
def pose_callback(msg):
    global try_again, index, try_again, index, result
    if msg.status.status == 3 and count>0 :  #成功到达任意目标点，前往下一目标点
        try_again = 1 #允许再次尝试前往尚未抵达的该目标点
       
        #count表示总目标点计数，index表示已完成的目标点计数
        if index == (count-1)*3:                   #当index等于count时，表示所有目标点完成，重新开始巡航
           ...
           # print('x:'+str(markerArray.markers[index-1].pose.position.x)+
            ...
            pose.pose.position.x = markerArray.markers[index_selector(count,index)].pose.position.x
            pose.pose.position.y = markerArray.markers[index_selector(count,index)].pose.position.y
            pose.pose.orientation.z = markerArray.markers[index_selector(count,index)].pose.orientation.z
            pose.pose.orientation.w = markerArray.markers[index_selector(count,index)].pose.orientation.w
            goal_pub.publish(pose)
            index += 1 #已完成的目标点计数加一

        elif index < (count-1)*3:                   #当index小于count时，表示还未完成所有目标点，目标巡航未完成
            print ('Reach the target point '+str(index_selector(count,index-1))+':')    
            print('x:'+str(markerArray.markers[index_selector(count,index-1)].pose.position.x)+
                ', y:'+str(markerArray.markers[index_selector(count,index-1)].pose.position.y)+
                ', z:'+str(markerArray.markers[index_selector(count,index-1)].pose.orientation.z)+
                ', w:'+str(markerArray.markers[index_selector(count,index-1)].pose.orientation.w)) 
        ...
        rospy.logwarn('Can not reach the target point '+str(index_selector(count,index-1))+':'+'\r\n'+
                      'x:'+str(markerArray.markers[index_selector(count,index-1)].pose.position.x)+
                    ', y:'+str(markerArray.markers[index_selector(count,index-1)].pose.position.y)+
                    ', z:'+str(markerArray.markers[index_selector(count,index-1)].pose.orientation.z)+
                    ', w:'+str(markerArray.markers[index_selector(count,index-1)].pose.orientation.w)) 
        ...
        if try_again == 1:
            rospy.logwarn('trying reach the target point '+str(index_selector(count,index-1))+' again!'+'\r\n'+
                          'x:'+str(markerArray.markers[index_selector(count,index-1)].pose.position.x)+
                        ', y:'+str(markerArray.markers[index_selector(count,index-1)].pose.position.y)+
                        ', z:'+str(markerArray.markers[index_selector(count,index-1)].pose.orientation.z)+
                        ', w:'+str(markerArray.markers[index_selector(count,index-1)].pose.orientation.w)) 
            ...
            pose.header.frame_id = 'map'
            pose.header.stamp = rospy.Time.now()
            pose.pose.position.x = markerArray.markers[index_selector(count,index-1)].pose.position.x           
            pose.pose.position.y = markerArray.markers[index_selector(count,index-1)].pose.position.y
            pose.pose.orientation.z = markerArray.markers[index_selector(count,index-1)].pose.orientation.z
            pose.pose.orientation.w = markerArray.markers[index_selector(count,index-1)].pose.orientation.w
            goal_pub.publish(pose)
            try_again = 0 #不允许再次尝试前往尚未抵达的该目标点

        #如果已经尝试过前往尚未抵达的目标点，则前往下一个目标点
        elif index < len(markerArray.markers*3):      #若还未完成目标点
            rospy.logwarn('try reach the target point '+str(index_selector(count,index-1))+' failed! reach next point:'+'\r\n'+
                          'x:'+str(markerArray.markers[index_selector(count,index-1)].pose.position.x)+
                        ', y:'+str(markerArray.markers[index_selector(count,index-1)].pose.position.y)+
                        ', z:'+str(markerArray.markers[index_selector(count,index-1)].pose.orientation.z)+
                        ', w:'+str(markerArray.markers[index_selector(count,index-1)].pose.orientation.w)) 

            if index==(count-1)*3-1: index=0 #如果下一个目标点序号为(count-1)*3-1，说明当前目标点为最后一个目标点，下一个目标点序号应该设置为0
            pose = PoseStamped()
            pose.header.frame_id = 'map'
            pose.header.stamp = rospy.Time.now()
            pose.pose.position.x = markerArray.markers[index_selector(count,index)].pose.position.x      
...
def navGoal_callback(msg):           
    ...
    marker_number.scale.x = 1 #marker大小
    marker_number.scale.y = 1 #marker大小
    marker_number.scale.z = 1 #marker大小，对于字符只有z起作用
...
def click_callback(msg):           
    ...
    marker.action = marker.ADD #添加marker
    marker.scale.x = 0.4 #marker大小
    marker.scale.y = 0.1 #marker大小
    marker.scale.z = 0.1 #marker大小，对于字符只有z起作用
    marker.color.a = 1 #字符透明度
    marker.color.r = 1 #字符颜色R(红色)通道
    marker.color.g = 0 #字符颜色G(绿色)通道
    marker.color.b = 0 #字符颜色B(蓝色)通道
    ...
    marker_number.color.g = 1 #字符颜色G(绿色)通道
