    ...
    if msg.status.status == 3 and count>0 :  #成功到达任意目标点，前往下一目标点
        ...
        if index == (count-1)*3:                   #当index等于count时，表示所有目标点完成，重新开始巡航
           ...
           # print('x:'+str(markerArray.markers[index-1].pose.position.x)+
            ...
            pose.pose.position.x = markerArray.markers[test(count,index)].pose.position.x
            pose.pose.position.y = markerArray.markers[test(count,index)].pose.position.y
            pose.pose.orientation.z = markerArray.markers[test(count,index)].pose.orientation.z
            pose.pose.orientation.w = markerArray.markers[test(count,index)].pose.orientation.w
            goal_pub.publish(pose)
            index += 1 #下一次要发布的目标点序号

        elif index < (count-1)*3:                   #当index小于count时，表示还未完成所有目标点，目标巡航未完成
            print ('Reach the target point '+str(test(count,index-1))+':')    
            print('x:'+str(markerArray.markers[test(count,index-1)].pose.position.x)+
                ', y:'+str(markerArray.markers[test(count,index-1)].pose.position.y)+
                ', z:'+str(markerArray.markers[test(count,index-1)].pose.orientation.z)+
                ', w:'+str(markerArray.markers[test(count,index-1)].pose.orientation.w)) 
        ...
        rospy.logwarn('Can not reach the target point '+str(test(count,index-1))+':'+'\r\n'+
                      'x:'+str(markerArray.markers[test(count,index-1)].pose.position.x)+
                    ', y:'+str(markerArray.markers[test(count,index-1)].pose.position.y)+
                    ', z:'+str(markerArray.markers[test(count,index-1)].pose.orientation.z)+
                    ', w:'+str(markerArray.markers[test(count,index-1)].pose.orientation.w)) 
        ...
        if try_again == 1:
            rospy.logwarn('trying reach the target point '+str(test(count,index-1))+' again!'+'\r\n'+
                          'x:'+str(markerArray.markers[test(count,index-1)].pose.position.x)+
                        ', y:'+str(markerArray.markers[test(count,index-1)].pose.position.y)+
                        ', z:'+str(markerArray.markers[test(count,index-1)].pose.orientation.z)+
                        ', w:'+str(markerArray.markers[test(count,index-1)].pose.orientation.w)) 
            ...
            pose.header.frame_id = 'map'
            pose.header.stamp = rospy.Time.now()
            pose.pose.position.x = markerArray.markers[test(count,index-1)].pose.position.x           
            pose.pose.position.y = markerArray.markers[test(count,index-1)].pose.position.y
            pose.pose.orientation.z = markerArray.markers[test(count,index-1)].pose.orientation.z
            pose.pose.orientation.w = markerArray.markers[test(count,index-1)].pose.orientation.w
            goal_pub.publish(pose)
            try_again = 0 #不允许再次尝试前往尚未抵达的该目标点

        #如果已经尝试过前往尚未抵达的目标点，则前往下一个目标点
        elif index < len(markerArray.markers*3):      #若还未完成目标点
            rospy.logwarn('try reach the target point '+str(test(count,index-1))+' failed! reach next point:'+'\r\n'+
                          'x:'+str(markerArray.markers[test(count,index-1)].pose.position.x)+
                        ', y:'+str(markerArray.markers[test(count,index-1)].pose.position.y)+
                        ', z:'+str(markerArray.markers[test(count,index-1)].pose.orientation.z)+
                        ', w:'+str(markerArray.markers[test(count,index-1)].pose.orientation.w)) 

            if index==(count-1)*3-1: index=0 #如果下一个目标点序号为count，说明当前目标点为最后一个目标点，下一个目标点序号应该设置为0
            pose = PoseStamped()
            pose.header.frame_id = 'map'
            pose.header.stamp = rospy.Time.now()
            pose.pose.position.x = markerArray.markers[test(count,index)].pose.position.x      
